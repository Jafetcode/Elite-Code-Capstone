  ...
  const fetchQuestions = async () => {
    try {
      const res = await fetch(
        `https://elitecodecapstone24.onrender.com/instructor/questions?cid=${cid}`
      );
    ...
    <Layout style={{ flex: 1, padding: 20, backgroundColor: "#2C496B" }}>
      <ScrollView>
        ...
        >
          ...
          <TouchableOpacity
            onPress={() => {
              navigation.navigate("TeacherCreateQuestion", {
                cid: cid,
              });
            }}
          >
              ...
              {questions.map(
                (question) =>
                  (question.classView === 1 || question.studentView === 1) && (
                    <Card key={question.qid} style={{ marginBottom: 8 }}>
                      <TouchableOpacity
                        onPress={() => {
                          navigation.navigate("TeacherManageQuestion", {
                            course: { cid },
                            qid: question.qid,
                          });
                        }}
                      >
                        ...
                        <View>
                          <Text category="s2"><Text>Topic: </Text>{question.topic}</Text>
                        </View>
                        <View>
                          <Text category="s2">
