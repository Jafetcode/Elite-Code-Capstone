  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
        <Text></Text>
        <TouchableOpacity
          onPress={() => navigation.navigate("TeacherCreateQuestion")}
        >
          <Text style={{ marginBottom: 10 }} category="s1" appearance="hint">
            Create Question
          </Text>
        </TouchableOpacity>
      </View>
          ...
          {questions.length > 0 ? (
            <>
              {questions.map((question) => (
                <Card
                  key={question.qid}
                  style={{ borderRadius: 10, borderWidth: 0, marginBottom: 10 }}
                >
                  <View
                    style={{
                      flexDirection: "row",
                  ...
                  <View>
                    <Text category="s2">{question.type}</Text>
                  </View>
                  <View>
                    <Text category="s2">
                      <Text category="s2">Topic: </Text>
                      {question.topic}
                    </Text>
                  </View>
                  <View>
                    <Text category="s2">
                      <Text category="s2">Due: </Text>
                      {formatDate(question.dueDate)}
                    </Text>
                  </View>
                  <View>
                    <Text category="s2">
                      <Text category="s2">{question.pointVal}</Text>
                      <Text category="s2"> Points</Text>
                    </Text>
                  </View>
                  <View style={styles.container}>
                    <Button
                      ...
                      onPress={() =>
                        navigation.navigate("Assign a question", {
                          assignTo: "course",
                          question: question,
                        })
                      }
                    >
                      <Text>Assign to class or student/students</Text>
                    </Button>
                  </View>
                </Card>
              ))}
